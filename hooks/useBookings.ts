    ...
    mutationFn: async (booking: {
      ...
      try {
        const { data: studioData } = await supabase
          .from("studios")
          .select(
            "name, studio_owner_profiles!owner_id (user_id)",
          )
...
export function useCheckIn() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!booking.qr_code || qrCode !== booking.qr_code)
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (booking.status !== "ACTIVE")
        throw new Error("Booking must be ACTIVE");
      if (booking.booker_confirmed_check_in)
      ...
      if (
        ...
        new Date() > new Date(booking.confirmation_expires_at)
      ) {
...
export function useCheckOut() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const totalScheduledMinutes = Math.round(
        (scheduledEnd.getTime() - scheduledStart.getTime()) / (1000 * 60),
      );
      const actualSessionMinutes = Math.max(
        1,
        Math.round((now.getTime() - checkedInAt.getTime()) / (1000 * 60)),
      );
      const isEarlyEnd = now < scheduledEnd;

      if (isEarlyEnd && !reason)
        throw new Error("Reason required for early session end");
      ...
      if (now > scheduledEnd) {
        ...
        overtimeAmount =
          (overtimeMinutes / 60) * parseFloat(booking.studios.hourly_rate);
      }
      ...
      if (isEarlyEnd) {
        const usageRatio = Math.min(
          actualSessionMinutes / totalScheduledMinutes,
          1,
        );
        proRataAmount = Math.round(totalAmount * usageRatio * 100) / 100;
      }

      const paymentReleaseEligibleAt = new Date(
  ...
  return useMutation({
    mutationFn: async (bookingId: string) => {
      ...
      return {
        ...
        finalAmount,
        platformFee,
        studioOwnerPayout: finalAmount - platformFee,
      };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bookings"] });
...
export function useRaiseDispute() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (booking.payment_status === "PAYMENT_RELEASED")
        throw new Error("Cannot dispute after payment released");
      if (
        booking.dispute_status === "OPEN" ||
        booking.dispute_status === "UNDER_REVIEW"
      ) {
